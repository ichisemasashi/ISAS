        ...
        (let [{:keys [app asid usid uid]} (farm sys)
          ...
          (testing "V2P1-2.5-05〜11 daily と重なり"
            (let [a (get-in (tu/parse (tu/post-json app "/api/user/gantt"
                  ...
                  around7 (tu/parse (tu/get-query app "/api/user/gantt/daily"
                                                  {:range "around7" :statuses "not_started,in_progress,done"}
                                                  "user" usid))
                  week (tu/parse (tu/get-query app "/api/user/gantt/daily"
                                               {:range "week" :statuses "not_started,in_progress,done"}
                                               "user" usid))
                  last-week (tu/parse (tu/get-query app "/api/user/gantt/daily"
                                                    {:range "last_week" :statuses "not_started,in_progress,done"}
                                                    "user" usid))
                  days7 (tu/parse (tu/get-query app "/api/user/gantt/daily"
                                                {:range "days7"} "user" usid))
              ...
              (is (true? (:overdue (first (filter #(= "境界開始" (:title %)) (:rows today))))))
              (is (false? (:overdue (first (filter #(= "今日内" (:title %)) (:rows today))))))
              (is (some #(= "7日内" (:title %)) (:rows around7)))
              (is (some #(= "境界終端" (:title %)) (:rows around7)))
              (is (false? (:overdue (first (filter #(= "7日内" (:title %)) (:rows around7))))))
              (is (some #(= "境界終端" (:title %)) (:rows week)))
              (is (some #(= "境界開始" (:title %)) (:rows week)))
              (is (not (some #(= "7日内" (:title %)) (:rows week))))
              (is (some #(= "7日内" (:title %)) (:rows last-week)))
              (is (not (some #(= "境界終端" (:title %)) (:rows last-week))))
              (is (= "range_invalid" (:code days7)))
              (is (:ok all))
...
(deftest v2p1-ui-handlers
  (testing "日次の読み込み・フィルタ・状態保存"
    (let [s0 (assoc (ui/init-state) :page :daily :kind "user" :session {:email "a"}
          ...
          set-last (ui/handle (assoc (:state loaded) :daily-rows [{:id 1}])
                              [:submit {:act "set-daily-range" :form {:range "last_week"}}])
          set-around (ui/handle (assoc (:state loaded) :daily-rows [{:id 1}])
                                [:submit {:act "set-daily-range" :form {:range "around7"}}])
      ...
      (is (re-find #"range=week" (nth (first (:fx set-week)) 2)))
      (is (re-find #"range=last_week" (nth (first (:fx set-last)) 2)))
      (is (re-find #"range=around7" (nth (first (:fx set-around)) 2)))
      ...
      (is (= "/api/user/gantt/1/status" (nth (first (:fx save)) 2)))
      (is (= {:execution_status "done"} (nth (first (:fx save)) 3)))
      ...
      (is (= "around7" (get-in path [:state :daily-range])))))
  ...
  (testing "time windows"
    (binding [time/*now-fn* (fn [] (Instant/parse "2026-09-21T01:00:00Z"))]
      ...
      (is (= ["2026-09-14T00:00" "2026-09-21T00:00"] (time/tokyo-last-week-window)))
      (is (= ["2026-09-14T00:00" "2026-09-29T00:00"] (time/tokyo-around7-window))))
    (binding [time/*now-fn* (fn [] (Instant/parse "2026-09-19T15:00:00Z"))]
      (is (= ["2026-09-14T00:00" "2026-09-21T00:00"] (time/tokyo-week-window)))
      (is (= ["2026-09-07T00:00" "2026-09-14T00:00"] (time/tokyo-last-week-window)))
      (is (= ["2026-09-13T00:00" "2026-09-28T00:00"] (time/tokyo-around7-window)))))
  (testing "cloverage 分岐"
    ...
    (is (re-find #"range=around7" (#'ui/daily-query-path {:daily-range "nope" :daily-statuses nil})))
    ...
    (is (re-find #"選んだ期間・状態に重なる作業がありません"
                 (html {:page :daily :fields [{:id 1}]
                        :daily-range "around7" :daily-statuses ["not_started"]
    ...
    (is (re-find #"range=around7" (#'ui/daily-query-path {})))
    (is (re-find #"range=around7" (#'ui/daily-query-path {:daily-range nil})))
    (is (re-find #"range=around7" (#'ui/daily-query-path {:daily-range "days7" :daily-statuses ["done"]})))
    (is (re-find #"range=all" (#'ui/daily-query-path {:daily-range "all"})))
    (is (re-find #"range=last_week" (#'ui/daily-query-path {:daily-range "last_week"})))
    (is (re-find #"<option value=\"around7\" selected"
                 (html {:page :daily :fields [{:id 1}]})))
