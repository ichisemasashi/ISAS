...
function fixture(status: "accepted" | "duplicate" | "rejected" | "conflict" = "accepted") {
  ...
  const api: MvpGateway = { getToday: vi.fn(), getFields: vi.fn(), getWorkInstructions: vi.fn(async () => ({ instructions: [] })), createWorkInstruction: vi.fn(), reassignWorkInstruction: vi.fn(), getJournalBootstrap: vi.fn(), getJournals: vi.fn(async () => ({ journals: [] })), reviewJournal: vi.fn(), uploadJournalAttachment: vi.fn(), push: vi.fn(async () => ({ results: [{ bundleId: "bundle-1", status, rejection: status === "rejected" ? { reason: "authorization_changed", recoveryAction: "manager_review" } : undefined }] })), pull: vi.fn(async () => ({ changes: [], nextCursor: "0", hasMore: false })), getQueues: vi.fn(async () => ({ rejections: [], conflicts: [] })), resolveConflict: vi.fn() };
