      ...
      <div className="page-shell">
        <header className="topbar">
          ...
          <div className="preferences">
            ...
            <button className="avatar" aria-label={`${authorization.user.displayName}のアカウントメニュー`} onClick={() => navigate("more")}>{authorization.user.initials}</button>
          </div>
        </header>
...
function InventoryPage({ api, authorization, online, storage, queue, navigate, setNotice }: { api: MvpGateway; authorization: AppAuthorization; online: boolean; storage: StorageGateway; queue: (kind: "stock", payload: Record<string, unknown>) => Promise<boolean>; navigate: (route: Route) => void; setNotice: (message: string) => void }) {
  ...
  return <div className="page-content narrow-page"><PageBack onBack={() => navigate("today")}/><div className="form-heading"><span className="section-kicker">INVENTORY</span><h1>在庫入出庫</h1><p>残高は直接編集せず、入出庫と棚卸し調整の履歴から計算します。</p></div>
    <section className="queue-panel"><h2>現在の在庫</h2>{snapshot.balances.length ? snapshot.balances.map((item) => <article key={item.chemicalId}><strong>{item.name}</strong><p>{item.registrationNumber}・残量 <b>{item.quantity}</b>{item.policy ? `・発注点 ${item.policy.reorderPoint}／目標 ${item.policy.targetLevel}` : ""}</p>{item.belowReorderPoint && <p role="alert">発注点以下です。入荷予定を確認してください。</p>}</article>) : <p>端末に在庫データがありません。オンラインで更新してください。</p>}</section>
    {!!snapshot.incoming?.length && <section className="queue-panel"><h2>入荷予定</h2>{snapshot.incoming.map((item) => <article key={item.purchaseOrderLineId || `${item.purchaseOrderId}-${item.chemicalId}`}><strong>{item.name || item.chemicalId}</strong><p>{item.orderNumber}・{item.supplierName}・残り {item.incomingQuantity}{item.unit}・予定 {item.expectedOn || "未定"}</p></article>)}</section>}
    {!!snapshot.lots?.length && <section className="queue-panel"><h2>ロット・期限・評価</h2>{snapshot.lots.map((lot) => <article key={lot.id}><strong>{lot.name || lot.chemicalId}／{lot.lotNumber}</strong><p>残量 {lot.quantity}{lot.unit}・期限 {lot.expiresOn || "設定なし"}・評価額 {lot.inventoryValue.toLocaleString("ja-JP")} {lot.currency}</p></article>)}</section>}
    {!!snapshot.counts?.length && <section className="queue-panel"><h2>棚卸し履歴</h2>{snapshot.counts.map((count) => <article key={count.id}><strong>{count.locationName}</strong><p>{new Date(count.countedAt).toLocaleString("ja-JP")}・{count.status}・差異合計 {count.absoluteVariance ?? 0}</p></article>)}</section>}
