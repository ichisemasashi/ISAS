  ...
  const provisionLocalTestUser = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const credential = await api.provisionLocalTestUser(contextId, csrfToken, {
        username: String(data.get("username") || ""), email: String(data.get("email") || ""), displayName: String(data.get("displayName") || ""), roleKey: String(data.get("roleKey") || "worker"),
      });
