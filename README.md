...
> **対外提供scope:** 圃場・指示・日誌・農薬・在庫のself-host／offline core。ISASは現在、KSAS同等・全面代替ではありません。農機自動日誌、drone、食味・収量連携、可変施肥、機械monitoring等は未提供です。

イチセ・スマート・アグリ・システム — 営農支援Webアプリケーション。

現場での入力（作業日誌・農薬記録・打刻）を核に、圃場GIS・作付計画・在庫を扱う。農機連携等は将来計画であり、利用可能な現行scopeへ含めない。オフライン前提・多テナント・多言語（将来の海外展開）を初期から設計に織り込む。ライセンスは MIT（[LICENSE](LICENSE)）。サービス実施範囲・データレジデンシー・法令遵守は運用者責任。
...
| 場所 | 内容 |
|---|---|
| [docs/農業営農支援システム_要求仕様書.md](docs/農業営農支援システム_要求仕様書.md) | 要求仕様 v1.1（確定） |
...
Viteの`/api`は開発時に`http://127.0.0.1:3000`へproxyする。認証BFF、当日作業、作業指示・割当、打刻補完、写真付き日誌、訂正・差し戻し、農薬マスタ・安全再判定、在庫イベント、S7 push/pull、圃場PostGIS、同期キューを同一オリジンで提供するBFFを先に起動する。MapLibreの日本向けオンライン背景はADR-0010で許可した地理院標準タイルへ直接接続する。オフライン背景は、BFFがrangeごとに担当割当を再検証するprivate PMTilesから明示取得し、IndexedDBへ保存する。背景を利用できなくても担当圃場ポリゴンを背景色上へ描画する。Phase 1全体の完了状況は[開発工程.md §6](docs/開発工程.md#6-マイルストーンと現在地)を参照する。
