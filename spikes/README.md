...
| 項目 | 次にやること |
|---|---|
| **S6（iOS／Android端末能力）** | 測定PWA、desktop診断、8プロファイル×11試験の証跡gateは完了。サポート下限／現行iPhoneとAndroid実機でbrowser／standalone、再起動、容量圧迫、OS更新、共有端末を測定する |
| **S9（端末暗号化／失効／鍵交代）** | PWAのcache/outbox別非抽出鍵、event content key二重wrap、rotation、暗号消去、quota、logout保護をChromiumでPASS。実iOS/Android、実KMS回復、実Cognito logoutは未実施 |
