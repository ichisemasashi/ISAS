...
export function artifactFragment(component, image, digest, sbom) {
  ...
  return { name: component, image, digest, reference: `${image}@${digest}`, sbom, signature: "cosign-aws-kms", provenance: "buildkit-slsa" };
}
...
export function buildManifest({ version, sourceCommit, runId, createdAt, artifacts }) {
  ...
  return { schema_version: 1, version, source_commit: sourceCommit, workflow_run_id: String(runId), created_at: createdAt, artifacts: [...artifacts].sort((a, b) => a.name.localeCompare(b.name)) };
}

async function main(argv) {
  if (argv[0] === "fragment" && argv.length === 6) {
    const fragment = artifactFragment(argv[1], argv[2], argv[3], argv[4]);
    await writeFile(argv[5], `${JSON.stringify(fragment, null, 2)}\n`, { flag: "wx" });
    return;
  }
  ...
  throw new Error("usage: build-artifact-manifest.mjs fragment COMPONENT IMAGE DIGEST SBOM OUTPUT | bundle DIRECTORY VERSION COMMIT RUN_ID OUTPUT");
}
