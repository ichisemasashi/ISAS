...
const evidence = () => ({
  ...
  recovery_set: { status: "PASS", id: "rs-20260815-abc", components: ["database", "session_context", "private_objects", "quarantine_archive", "shard_config", "offline_maps", "queues", "audit", "configuration", "kms"], pitr_lag_seconds: 120, object_inventory_age_seconds: 3600,
    backup_jobs: ["database", "session_context", "private_objects", "quarantine_archive", "shard_config", "offline_maps"].map((component, i) => ({ component, status: "COMPLETED", recovery_point_arn: `arn:aws:backup:::${i}` })), evidence: "s3://evidence/recovery.json" },
