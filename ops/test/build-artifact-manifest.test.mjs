...
const signingKey = "arn:aws:kms:ap-northeast-1:123456789012:key/11111111-1111-1111-1111-111111111111";

test("build manifest requires one immutable artifact for every implemented component", () => {
  const artifacts = ["web", "bff", "migration"].map((name) => artifactFragment(name, `123.dkr.ecr.ap-northeast-1.amazonaws.com/isas-jp-stg/${name}`, digest, `${name}.spdx.json`, `${name}.cosign.pub`, digest, signingKey));
...
test("build manifest rejects tags and incomplete artifact sets", () => {
  assert.throws(() => artifactFragment("bff", "registry/isas/bff", "latest", "bff.spdx.json", "bff.cosign.pub", digest, signingKey), /digest/);
