...
| KCOMP2-M1 | Medium | Docker撤去 | ADR-0024は採用済みだが、local-integration、CI、任意AWS adapterの3群がactive transitionalである。R1 DB spikeはnative代替へ移行済み | R1 native PG/PostGIS runner→R2 native local-integration→R3 native CI→R4 AWS adapter→R5物理削除の順で、代替合格後に旧経路を削除する | **R1・R2対応済み／R3着手可（R4〜R5は順序待ち）**。R2はmacOS arm64の10 launchd user agentでPG16.15＋PostGIS 3.6.4、5 PgBouncer、Keycloak、OTel、BFF、loopback HTTPSを起動し、native構成8/8、BFF 117/117、DB／RLS／owner／監査、OIDC／MFA／業務E2E 4/4、stop／start永続性を受入後に旧Compose local経路を削除。証跡：`ops/docker-retirement/evidence/R1-native-pg-acceptance.json`、`ops/docker-retirement/evidence/R2-native-local-integration-acceptance.json` |
