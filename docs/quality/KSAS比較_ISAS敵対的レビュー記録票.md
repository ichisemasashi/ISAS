...
| KCOMP2-M1 | Medium | Docker撤去 | ADR-0024は採用済みだがR0のみ完了し、local-integration、DB spike、CI、任意AWS adapterの4群がactive transitionalのままである。daemon障害が開発・検証を再び停止できる | R1 native PG/PostGIS runner→R2 native local-integration→R3 native CI→R4 AWS adapter→R5物理削除の順で、代替合格後に旧経路を削除する | **R0完了・R1 runner実装済み／PG16用PostGIS導入待ち・R2〜R5未着手** |
