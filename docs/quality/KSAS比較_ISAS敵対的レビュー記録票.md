...
| KCOMP2-M1 | Medium | Docker撤去 | ADR-0024は採用済みだが、local-integration、CI、任意AWS adapterの3群がactive transitionalである。R1 DB spikeはnative代替へ移行済み | R1 native PG/PostGIS runner→R2 native local-integration→R3 native CI→R4 AWS adapter→R5物理削除の順で、代替合格後に旧経路を削除する | **R1対応済み・R2着手可（R3〜R5は順序待ち）**。PG16.15＋PostGIS 3.6.4でmigration／verify／rollback／S1／S2／S5／S7／S8を受入後、旧Compose spikeを削除。証跡：`ops/docker-retirement/evidence/R1-native-pg-acceptance.json` |
