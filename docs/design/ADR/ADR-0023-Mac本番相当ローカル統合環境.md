...
| 項目 | 内容 |
|---|---|
| ステータス | **採用（クローズ v5）**（native launchd orchestrationへ移行済み。環境の責任境界と共通core／local adapterを維持） |
| 日付 | 2026-08-16 |
...
> **v5実装状態**：2026-08-28に[ADR-0024](ADR-0024-Docker段階的撤去.md) R2を受入し、Compose経路を削除した。以下はnative process、launchd user agent、version固定native package／application artifactを現在の正本とする。
...
- orchestrationは非特権のlaunchd user agentとnative process監督とする。`ops/local/local-{up,status,stop,restart,reset}.sh`はnative lifecycleの標準入口であり、Docker daemonへ依存しない。
...
| 責務 | 採用 | 契約／理由 |
|---|---|---|
| Orchestration | **launchd user agent＋native process** | version管理したservice定義、health、data directory、profile、resource境界を維持する。10個のuser agentを`gui/<uid>` domainで監督し、root権限を要求しない |
...
| 権威DB | PostgreSQL 16＋PostGIS 3.6.4 | production採用majorと揃え、正式migration、RLS／FORCE、owner、trigger、`security_invoker`、GISを同じSQLで検証する |
| DB pool | PgBouncer **1.25.2**を5独立instance | P0／Auth-P1／P1／P2／Opsごとにprocess、port、DB role、connection上限、timeoutを分ける。1.25.2未満は禁止し、単一processの別portは障害・上限分離を偽装するため不採用 |
...
| Telemetry | OpenTelemetry Collector contrib 0.133.0 | BFFのOTLP受信とCollector自身のmetrics endpointをnative processで検証する。dashboard backendは本R2の必須runtimeに含めず、auditはDB権威経路のまま一般logへ置換しない |
...
1. browserへ公開するのはCaddyの`127.0.0.1:8443`だけとする。PostgreSQL、5 PgBouncer、Keycloak、telemetryは固定loopback portにbindし、LAN interfaceへ公開しない。
2. service間通信はloopbackの固定portだけを使う。runtimeからinternetへ出ることを前提にせず、地図、email、webhookはsynthetic local fixture／sinkへ固定する。
3. BFF起動時にDB、issuer、object、queue、OTLPのscheme／host／portとcredential sourceを検査し、native loopbackの署名済みtest allowlist以外を拒否する。denylistだけに依存しない。
...
7. OIDC issuerはbrowserとBFFで同じ`https://isas.localhost:8443/oidc/...`を使う。BFFのdiscovery／JWKS／token通信もCaddy経由にし、Keycloakのloopback portを別issuerとして使わない。mkcert root CAはBFF processの追加trust storeへ限定登録する。
...
- key fileは`~/Library/Application Support/ISAS/local-integration/secrets/`へ生成し、directory `0700`、file `0600`を検査する。repository外へ分離した上でtracked file検査とsecret scanを行う。
...
- host prerequisiteはmacOS、Node.js 22、native PostgreSQL／PostGIS／PgBouncer／Caddy、mkcert、空きdisk／memoryである。Keycloak、Temurin、OpenTelemetry Collectorはarchitecture別URLとSHA-256をlockし、repository外へ検証付きで配置する。
- root権限とhost filesystem全体へのアクセスを禁止し、launchd user agentと用途別data rootでprocess境界を持つ。
- native artifactはversion、source／license／SBOMを台帳化し、Critical／High vulnerability 0をacceptance条件にする。lock更新は自動適用せず、checksum、起動、migration、E2E、security test後にreviewする。
...
| Kubernetes／Compose | Mac反復環境にはresource／daemon依存が過大で、ユーザー要求でもない | native launchdを採用 |
...
- 全serviceは同じMac、filesystem、電源にあり、HA、failure domain、RPO/RTOを証明しない。
...
1. 【v5実装済み】`component-lock`、doctor、secret bootstrap、Caddy／Keycloak／PG／5 PgBouncer／telemetryのnative launchd foundation
...
9. 【v5実装済み】ADR-0024 R2として同じ操作契約をlaunchd user agentとnative依存へ移し、受入合格後にCompose foundationを削除
